...
/// <summary>
/// DI registration for the HITL Coordination library (engineering-standards: each
/// library wires its own internals; only Host calls these extensions).
/// </summary>
public static class HitlServiceCollectionExtensions
{
    /// <summary>
    /// Binds and validates <see cref="CosmosOptions"/> (doc 12 §4 "options with teeth"),
    /// and registers the <see cref="CosmosClient"/>, <see cref="IApprovalStore"/>,
    /// <see cref="IApprovalQueryService"/>, <see cref="RequestApprovalActivity"/>,
    /// <see cref="EscalateApprovalActivity"/>, <see cref="IRollbackPlanner"/>, and the
    /// <see cref="CosmosTopologyCheck"/> boot invariant (doc 12 §6).
    /// </summary>
    public static IServiceCollection AddFrontierHitl(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        return services
            ...
            .AddSingleton<DecidedApprovalQuery>()
            .AddSingleton<IApprovalStore, CosmosApprovalStore>()
            .AddSingleton<IApprovalQueryService, ApprovalQueryService>()
            .AddSingleton<IRollbackPlanner, RollbackPlanner>()
